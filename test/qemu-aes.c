...
static uint64_t target_inst_gfn = 0;
...
monitor(bool baseline)
{
	...
	case CPC_EVENT_TRACK_STEP:
		...
		if (!target_inst_gfn && baseline)
			target_inst_gfn = event.step.inst_gfn;
...
read_batch(void)
{
	...
	for (i = 0; i < batch.cnt; i++) {
		if (batch.buf[i].type == CPC_EVENT_TRACK_PAGE) {
			...
			target_inst_gfn = batch.buf[i].page.inst_gfn;
		} else if (batch.buf[i].type == CPC_EVENT_TRACK_STEP) {
			printf("INST %08llx %04u\n",
				batch.buf[i].step.inst_gfn,
				batch.buf[i].step.fault_count);
		}
	}
}
...
main(int argc, const char **argv)
{
	...
	memset(&cfg, 0, sizeof(cfg));
	cfg.mode = CPC_TRACK_STEPS;
	cfg.steps.target_gfn = target_inst_gfn;
