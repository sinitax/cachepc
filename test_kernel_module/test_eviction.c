...
main(int argc, const char **argv)
{
	...
	int16_t single_eviction_access[SAMPLE_COUNT][64];
	for(int i=0; i < SAMPLE_COUNT; ++i){

		int fd;
		fd = open("/proc/cachepc", O_WRONLY);
		write(fd, "a",1);
		close(fd);
		int16_t *tmp_res =  print_accessed_sets();
		memcpy(single_eviction_access[i], tmp_res, 64*sizeof(int16_t));
		free(tmp_res);
	}
	printf("####  Single eviction test ####\n");
	for(int i=0; i<SAMPLE_COUNT; ++i){
		printf("Sample number %d:\n", i);
		for(int j=0; j<64; ++j){
			printf("%3d ", single_eviction_access[i][j]);
		}
		putchar('\n');
	}
	printf("\n");
	dump_msrmt_results_to_log("single_eviction_test.out", single_eviction_access);
